...
int compare_diff_spj(const char* spj_exe, const char* file1, const char* file2)
{
    int pid = fork();
    if(pid == 0)
    {
        execl(spj_exe, spj_exe, file1, file2, NULL);
        exit(-1);
    }
    else if(pid > 0)
    {
...
{
    return compare_diff_spj(spj_exe, file1, file2);
}